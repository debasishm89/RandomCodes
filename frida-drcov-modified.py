...
def populate_bbs(data):
    ...
    for i in range(0, len(data), block_sz):
        mydata = data[i:i+block_sz]
        start = struct.unpack('I',mydata[:4])[0] # ABCD
        size = struct.unpack('H',mydata[4:-2])[0] # EF
        mod_id = struct.unpack('H',mydata[6:])[0] #GH
        buff = 'module[ '+str(mod_id)+']: '+str(hex(start))+', '+str(size)+'\n'
        bbs.add(buff)
